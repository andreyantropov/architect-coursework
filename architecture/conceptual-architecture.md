...
| Логи                  | Elasticsearch       | Быстрый поиск и анализ логов                                              |
...
## Мобильная платформа (MP) и фронтенд
...
- **DMZ Zone**:
  - Изолированная сеть для публично доступных компонентов
  - WAF (Web Application Firewall) перед External API Gateway
  - DDoS protection сервисы
- **Двойной API Gateway**:
  - External: Оптимизирован для клиентских запросов, кэширования
  - Internal: Оптимизирован для межсервисного взаимодействия
  - Analytics: Специализирован для тяжелых аналитических запросов
- **Улучшенная безопасность**:
  - Двухфакторная аутентификация для административных функций
  - Раздельные ключи шифрования для DMZ и внутренней сети
  - Детальное логгирование всех межзоновых запросов

```json
// Интерфейс объекта логов
{
  "measurement": "string",
  "timestamp": 0,
  "tags": {
    ...
    "host": "string",
    "zone": "dmz | internal",
    "gatewayType": "external | internal | analytics"
  },
  "fields": {
    "id": "UUID",
    "message": "string",
